...
uchar BitBoard::generateOpponentMoves(MoveList & moveList)
{
  return 0;
}

uchar BitBoard::generateMoves(uchar row, uchar col, MoveList & moveList)
{
  return 0;
}
...
void BitBoard::initBoard()
{
  ...
  mColorBB[White] = 0;
  for (uchar i = WhiteKing; i <= WhitePawn; i++)
    mColorBB[White] |= mPieceBB[i];
  ...
  mColorBB[Black] = 0;
  for (uchar i = BlackKing; i <= BlackPawn; i++)
    mColorBB[Black] |= mPieceBB[i];
...
bool BitBoard::isCellAttacked(uchar row, uchar col, char attackingColor) const
{
  return false;
}

uchar BitBoard::kingRow(char color) const
{
  return 0;
}

uchar BitBoard::kingCol(char color) const
{
  return 0;
}

void BitBoard::makeMove(const Move & newMove)
{
  ...
  uchar bbIndex = newMove.toPiece + mSideToMove*WhitePawn + 1;
  ...
  if (newMove.fromPiece == King) {
    uchar kingType = (mSideToMove == White) ? WhiteKing : BlackKing;
    mKingIndex[mSideToMove] = toSquare;
    mPieceBB[kingType] ^= fromToBB;
  }
...
void BitBoard::pushMove(uchar fromSquare, uchar toSquare, uchar fromPiece, uchar capturePiece, char flags, MoveList & moveList)
{
...
void BitBoard::unmakeMove(const Move & undoMove)
{
...
void BitBoard::writeBitBoard(ulonglong bb) const
{
